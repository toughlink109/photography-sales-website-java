...
<div :style='{"width":"100%","padding":"0 10%","margin":"10px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="话题分类" prop="huatifenlei">
            <el-select v-model="ruleForm.huatifenlei" placeholder="请选择话题分类" :disabled=" false  ||ro.huatifenlei" >
              <el-option
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/huatifenlei/huatifenlei', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.huatifenleiOptions = res.data.data;
          }
        });
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`sheyinghuati/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('sheyinghuati/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`sheyinghuati/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
