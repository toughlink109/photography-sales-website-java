	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="话题分类" prop="huatifenlei">
					<el-select :disabled="ro.huatifenlei" v-model="ruleForm.huatifenlei" placeholder="请选择话题分类" >
						<el-option
							v-for="(item,index) in huatifenleiOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="话题分类" prop="huatifenlei">
					<el-input v-model="ruleForm.huatifenlei"
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='huatifenlei'){
							this.ruleForm.huatifenlei = obj[o];
							this.ro.huatifenlei = true;
							continue;
						}
						if(o=='fengmian'){
            ...
            this.$http({
				url: `option/huatifenlei/huatifenlei`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.huatifenleiOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springboottvfo8kua/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `sheyinghuati/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.sheyinghuatiCrossAddOrUpdateFlag = false;
											this.parent.search();
