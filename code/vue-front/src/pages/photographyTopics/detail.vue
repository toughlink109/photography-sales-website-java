...
<div>
	<div :style='{"border":"5px solid #FFC174","width":"80%","padding":"25px 30px","margin":"10px auto","borderRadius":"30px","background":"linear-gradient(180deg, #10615F 0%, #0A4742 100%)"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'Ξ'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/photographyTopics"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"border":"5px solid #FFC174","padding":"10px","borderRadius":"30px","background":"#fff","display":"flex","width":"45%","position":"relative","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0","margin":"0","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"width":"102px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#818181","textAlign":"right"}'>话题分类</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.topicCategories}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"width":"102px","padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#818181","textAlign":"right"}'>发布时间</div>
					<div  :style='{"padding":"0 10px","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.publishTime}}</div>
				</div>
				<div class="item" :style='{"padding":"10px","margin":"0 0 10px 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFC174","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('photographyTopics','修改')&&detail.isApproved!='是'&&detail.isApproved!='否' " @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFC174","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('photographyTopics','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFC174","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('photographyTopics','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"#FFC174","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('photographyTopics','审核') && (detail.isApproved=='待审核'||!detail.isApproved)" @click="onSH" type="warning">审核</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      favorites(type) {
        if (type == 1 && !this.isFavorites) {
          ...
          this.$http.post('favorites/add', this.favoritesParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('photographyTopics/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isFavorites) {
          this.$http.get('favorites/list', {params: {page: 1, limit: 1, type: 1, refId: this.detail.id, tableName: 'photographyTopics', userId: localStorage.getItem('frontUserId')}}).then(res => {
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("frontToken")) {
            this.$http.get('favorites/list', {params: {page: 1, limit: 1, type: 21, refId: this.detail.id, tableName: 'photographyTopics', userId: localStorage.getItem('frontUserId')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('favorites/list', {params: {page: 1, limit: 1, type: 22, refId: this.detail.id, tableName: 'photographyTopics', userId: localStorage.getItem('frontUserId')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      nextClick(page) {
		...
		backClick(){
			...
			this.$router.push({path: '/index/photographyTopics', query: params});
		},
		// 下载
		download(file){
			if(!file) {
      ...
      getDiscussList(page) {
        this.$http.get('photographyTopicComments/list', {params: {page, limit: this.pageSize, refId: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('photographyTopicComments/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('photographyTopicComments/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		...
		// 修改
		editClick(){
			this.$router.push(`/index/photographyTopicsAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此摄影话题？')
			  .then(_ => {
			    this.$http.post('photographyTopics/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
