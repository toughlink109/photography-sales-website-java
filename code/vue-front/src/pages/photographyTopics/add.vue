...
<div :style='{"width":"100%","padding":"0 10%","margin":"10px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="话题分类" prop="topicCategories">
            <el-select v-model="ruleForm.topicCategories" placeholder="请选择话题分类" :disabled=" false  ||ro.topicCategories" >
              <el-option
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/topicCategories/topicCategories', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.topicCategoriesOptions = res.data.data;
          }
        });
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`photographyTopics/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossRefId && crossUserId) {
						...
						this.$http.get('photographyTopics/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`photographyTopics/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
