...
<div :style='{"width":"100%","padding":"0 10%","margin":"10px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="作品" v-if="type!='cross' || (type=='cross' && !ro.work)" prop="work">
            <file-upload
            tip="点击上传作品"
            action="file/upload"
            :limit="3"
            :multiple="true"
            :fileUrls="ruleForm.work?ruleForm.work:''"
            @change="workUploadChange"
            ></file-upload>
          </el-form-item>
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="作品" prop="work">
                <img v-if="ruleForm.work.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.work.split(',')[0]" width="100" height="100">
                <img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.work.split(',')" :src="baseUrl+item" width="100" height="100">
            </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="发布时间" prop="publishTime">
              <el-date-picker
				  :disabled=" false  ||ro.publishTime"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="作品描述" prop="workDescription">
            <editor 
                ...
                v-model="ruleForm.workDescription" 
                class="editor" 
                action="file/upload">
            </editor>
          </el-form-item>
...
<script>
  export default {
    ...
    created() {
      ...
      this.ruleForm.publishTime = this.getCurDate()
    },
    methods: {
      getMakeZero(s) {
          return s < 10 ? '0' + s : s;
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='work'){
              this.ruleForm.work = obj[o].split(",")[0];
              this.ro.work = true;
              continue;
            }
            if(o=='workDescription'){
              this.ruleForm.workDescription = obj[o];
              this.ro.workDescription = true;
              continue;
            }
            if(o=='publishTime'){
              this.ruleForm.publishTime = obj[o];
              this.ro.publishTime = true;
              continue;
            }
            if(o=='account'){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`workPublish/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossRefId && crossUserId) {
						...
						this.$http.get('workPublish/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`workPublish/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
      ...
      workUploadChange(fileUrls) {
          this.ruleForm.work = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
      },
    }
  };
</script>
