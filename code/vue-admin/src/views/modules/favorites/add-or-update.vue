	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="推荐类型" prop="recommendType">
					<el-input v-model="ruleForm.recommendType" placeholder="推荐类型" clearable  :readonly="ro.recommendType"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="推荐类型" prop="recommendType">
					<el-input v-model="ruleForm.recommendType" placeholder="推荐类型" readonly></el-input>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='recommendType'){
							this.ruleForm.recommendType = obj[o];
							this.ro.recommendType = true;
							continue;
						}
						if(o=='remark'){
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `favorites/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.favoritesCrossAddOrUpdateFlag = false;
											this.parent.search();
