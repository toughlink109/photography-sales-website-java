	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="话题分类" prop="topicCategories">
					<el-select :disabled="ro.topicCategories" v-model="ruleForm.topicCategories" placeholder="请选择话题分类" >
						<el-option
							v-for="(item,index) in topicCategoriesOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="话题分类" prop="topicCategories">
					<el-input v-model="ruleForm.topicCategories"
						placeholder="话题分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="发布时间" prop="publishTime">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.publishTime" 
						type="datetime"
						:readonly="ro.publishTime"
						placeholder="发布时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.publishTime" label="发布时间" prop="publishTime">
					<el-input v-model="ruleForm.publishTime" placeholder="发布时间" readonly></el-input>
				</el-form-item>
...
export default {
	...
	created() {
		this.ruleForm.publishTime = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='topicCategories'){
							this.ruleForm.topicCategories = obj[o];
							this.ro.topicCategories = true;
							continue;
						}
						if(o=='coverImage'){
            ...
            this.$http({
				url: `option/topicCategories/topicCategories`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
					this.topicCategoriesOptions = data.data;
				} else {
					this.$message.error(data.msg);
				}
            });
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `photographyTopics/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.photographyTopicsCrossAddOrUpdateFlag = false;
											this.parent.search();
