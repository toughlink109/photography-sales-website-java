	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="性别" prop="userSex">
					<el-select :disabled="ro.userSex" v-model="ruleForm.userSex" placeholder="请选择性别" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="性别" prop="userSex">
					<el-input v-model="ruleForm.userSex"
						placeholder="性别" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="手机" prop="mobile">
					<el-input v-model="ruleForm.mobile" placeholder="手机" clearable  :readonly="ro.mobile"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="手机" prop="mobile">
					<el-input v-model="ruleForm.mobile" placeholder="手机" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="邮箱" prop="email">
					<el-input v-model="ruleForm.email" placeholder="邮箱" clearable  :readonly="ro.email"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="邮箱" prop="email">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-if="type!='info' && !ro.userAvatar" label="头像" prop="userAvatar">
					<file-upload
						tip="点击上传头像"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="upload" v-else-if="ruleForm.userAvatar" label="头像" prop="userAvatar">
					...
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.userAvatar.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
...
export default {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='userAvatar'){
							this.ruleForm.userAvatar = obj[o];
    ...
    // 提交
    onSubmit() {
	if(this.ruleForm.userAvatar!=null) {
		this.ruleForm.userAvatar = this.ruleForm.userAvatar.replace(new RegExp(this.$base.url,"g"),"");
	}
var objcross = this.$storage.getObj('crossObj');
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `ordinaryUser/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.ordinaryUserCrossAddOrUpdateFlag = false;
											this.parent.search();
    ...
    userAvatarUploadChange(fileUrls) {
	    this.ruleForm.userAvatar = fileUrls;
    },
  }
};
</script>
