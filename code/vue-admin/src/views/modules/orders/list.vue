<template>
	<div class="main-content" :style='{"padding":"30px","margin":"0"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#374254","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">订单编号</label>
						<el-input v-model="searchForm.orderId" placeholder="订单编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#374254","display":"inline-block","lineHeight":"40px","fontSize":"14px","fontWeight":"600","height":"40px"}' class="item-label">商品名称</label>
						<el-input v-model="searchForm.productTitle" placeholder="商品名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('orders'+'/'+orderStatus,'日销量')" type="success" @click="dayNumberChartDialog()">
						<span class="icon iconfont icon-xiaoliang1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						日销量
					</el-button>
					...
					<el-button class="btn18" v-if="isAuth('orders'+'/'+orderStatus,'品销量')" type="success" @click="productTitleNumberChartDialog()">
						<span class="icon iconfont icon-xiaoliang1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						商品销量
					</el-button>
					...
					<el-button class="btn18" v-if="isAuth('orders'+'/'+orderStatus,'日销额')" type="success" @click="dayAmountChartDialog()">
						<span class="icon iconfont icon-hexiao15" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						日销额
					</el-button>
					...
					<el-button class="btn18" v-if="isAuth('orders'+'/'+orderStatus,'品销额')" type="success" @click="productTitleAmountChartDialog()">
						<span class="icon iconfont icon-geren14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
						商品销额
					</el-button>
			...
			<div :style='{"width":"100%","padding":"10px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="isApproved" label="退货审核">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'物流')&&scope.row.id" type="success" @click="logisticsUpdate(scope.row.id)">
								<span class="icon iconfont icon-wuliu14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								物流
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'发货')&&scope.row.id" type="success" @click="updateHandler(scope.row)">
								<span class="icon iconfont icon-fahuo16" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								发货
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'确认收货')&&scope.row.id" type="success" @click="updateHandler2(scope.row)">
								<span class="icon iconfont icon-queren20" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								确认收货
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'退货审核')&&scope.row.isApproved=='待审核'" type="success" @click="returnSh(scope.row)">
								<span class="icon iconfont icon-zhangjie8" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"40px"}'></span>
								退货审核
							</el-button>
		...
		<el-dialog title="退货审核" :visible.sync="returnShVisible" width="50%">
			<el-form ref="returnShForm" :model="returnShForm" label-width="80px" :rules="returnShRules">
				<el-form-item label="审核状态" prop="isApproved">
					<el-select v-model="returnShForm.isApproved" placeholder="审核状态">
				...
				<el-form-item label="内容" prop="approvedComments">
					<el-input type="textarea" :rows="8" v-model="returnShForm.approvedComments"></el-input>
				</el-form-item>
			</el-form>
		...
		<el-dialog
		  title="商品销量"
		  :visible.sync="productTitleNumberChartVisiable"
		  width="800">
			<div id="productTitleNumberChart" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
			<el-button @click="productTitleNumberChartDialog">返回</el-button>
		  </span>
		</el-dialog>
		...
		<el-dialog
		  title="商品销额"
		  :visible.sync="productTitleAmountChartVisiable"
		  width="800">
			<div id="productTitleAmountChart" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
			<el-button @click="productTitleAmountChartDialog">返回</el-button>
		  </span>
		</el-dialog>
...
//导出excel
				json_fields: {
					...
					"物流": "logistics",    //常规字段
					"备注": "remark",    //常规字段
					"是否审核": "isApproved",    //常规字段
					"审核回复": "approvedComments",    //常规字段
					"用户角色": "role",    //常规字段
				},
				json_meta: [
					[
		...
		methods: {
			...
			orderStatusFormatter: function(row, column) {
				var temp = row.status
				if(row.status=='已退款'&&row.isApproved!=''){
					temp = '已退货'
				}
				return temp
			},
			...
			returnShSave(){
				this.$refs["returnShForm"].validate(valid => {
					if(valid){
						if(this.returnShForm.isApproved=="是"){
							this.returnShForm.status = '已退款'
							this.$http({
								url: `${this.returnShForm.tableName}/info/${this.returnShForm.productId}`,
								method: 'get',
							}).then(res=>{
								if(res.data&&res.data.code==0){
									...
									// 商品存在库存。加回库存
									if(good.goodsStocks){
										good.goodsStocks = parseInt(good.goodsStocks) + parseInt(this.returnShForm.quantity)
										this.$http({
						...
						}else if(this.returnShForm.isApproved=='否'){
							this.$http({
								url: 'orders/update',
								method: 'post',
								data: this.returnShForm
		...
		// 日销量
			dayNumberChartDialog() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `orders/value/addTime/quantity/日`,
						method: "get"
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
		...
		// 月销量
			monthNumberChartDialog() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `orders/value/addTime/quantity/月`,
						method: "get"
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			yearNumberChartDialog() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `orders/value/addTime/quantity/年`,
						method: "get"
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			productTitleNumberChartDialog() {
				this.productTitleNumberChartVisiable = !this.productTitleNumberChartVisiable;
				this.$nextTick(()=>{
					var productTitleNumberChart = echarts.init(document.getElementById("productTitleNumberChart"),'macarons');
					this.$http({
						url: `orders/value/productTitle/quantity`,
						method: "get"
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							let pArray = []
							for(let i=0;i<res.length;i++){
								xAxis.push(res[i].productTitle);
								yAxis.push(parseFloat(res[i].total));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].productTitle
								})
								var option = {};
								option = {
									tooltip: {
								...
								// 使用刚指定的配置项和数据显示图表。
								productTitleNumberChart.setOption(option);
								//根据窗口的大小变动图表
								window.onresize = function() {
									productTitleNumberChart.resize();
								};
							}
						}
					});
			...
			goodtypeNumberChartDialog() {
				...
				this.$nextTick(()=>{
					...
					this.$http({
						url: `orders/value/goodtype/quantity`,
						method: "get"
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
    ...
    // 月销额
			...
			productTitleAmountChartDialog() {
				this.productTitleAmountChartVisiable = !this.productTitleAmountChartVisiable;
				this.$nextTick(()=>{
					var productTitleAmountChart = echarts.init(document.getElementById("productTitleAmountChart"),'macarons');
					this.$http({
						url: `orders/value/productTitle/total`,
						method: "get"
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res.length;i++){
								...
								// 使用刚指定的配置项和数据显示图表。
								productTitleAmountChart.setOption(option);
								//根据窗口的大小变动图表
								window.onresize = function() {
									productTitleAmountChart.resize();
								};
							}
						}
					});
    ...
    getDataList() {
      ...
      let params = {
        ...
        status: this.$route.params.status,
           ...
           if(this.searchForm.productTitle!='' && this.searchForm.productTitle!=undefined){
            params['productTitle'] = '%' + this.searchForm.productTitle + '%'
          }
