	...
	<div class="addEdit-block" :style='{"padding":"30px","margin":"0"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="作品分类" prop="workCategory">
					<el-input v-model="ruleForm.workCategory" placeholder="作品分类" clearable  :readonly="ro.workCategory"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="作品分类" prop="workCategory">
					<el-input v-model="ruleForm.workCategory" placeholder="作品分类" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="投票票数" prop="voteCount">
					<el-select :disabled="ro.voteCount" v-model="ruleForm.voteCount" placeholder="请选择投票票数" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="投票票数" prop="voteCount">
					<el-input v-model="ruleForm.voteCount"
...
var objcross = this.$storage.getObj('crossObj');
      ...
      if(objcross!=null) {
	      ...
	      objcross.voteCount = parseFloat(objcross.voteCount) + parseFloat(this.ruleForm.voteCount)
                }
      //更新跨表属性
       var crossUserId;
       var crossRefId;
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `workVote/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.workVoteCrossAddOrUpdateFlag = false;
											this.parent.search();
