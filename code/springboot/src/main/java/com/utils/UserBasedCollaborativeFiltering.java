...
public class UserBasedCollaborativeFiltering {
    ...
    private Map<String, List<String>> itemAdmins;
    ...
    public UserBasedCollaborativeFiltering(Map<String, Map<String, Double>> userRatings) {
        ...
        for (String user : userRatings.keySet()) {
            ...
            for (String item : ratings.keySet()) {
                if (!itemAdmins.containsKey(item)) {
                    itemAdmins.put(item, new ArrayList<>());
                }
                itemAdmins.get(item).add(user);
            }
          //用户ID与稀疏矩阵建立对应关系
        ...
        for(String item : itemAdmins.keySet()) {
        	List<String> userList = itemAdmins.get(item);
    ...
    public List<String> recommendItems(String targetUser, int numRecommendations) {
        ...
        // 选择相似度最高的K个用户
        List<String> similarAdmins = new ArrayList<>();
        for (int i = 0; i < numRecommendations; i++) {
            if (i < sortedSimilarities.size()) {
                similarAdmins.add(sortedSimilarities.get(i).getKey());
            } else {
                break;
            }
        }
